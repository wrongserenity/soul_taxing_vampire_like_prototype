...
3. За границей экрана начинают спавниться враги "клоуны-петарды"
...
## 1.4. Фишка
Абилка-нить (push/pull) контроля за марианетками - универсальный инструмент:
1. Контролит толпу (оттолкнуть слишком сильных / притянуть слабых)
2. Решает простые сценические пазлы: достать прожектор из песка, убрать мешающие ящики, закрыть путь для противников
...
1. Персонаж игрока - заводная кукла
   - акцент: "ниточки" в движении
   - при получении абилки: ключик/катушка на спине
2. Враги - клоуны-петарды (заводные игрушки)
   - акцент: фитиль/мигание в зарядке, "тик-тик" звук
   - при начале детанации звук фитиля "пшш"
3. Якоря - прожекторные столбы
   - акцент: столб со световой техникой, четыре прожектора в разные стороны
   - по мере получения урона: отключаются прожекторы
4. Абилка - катушка/крюк на нити
   - акцент: нить натягивается до цели, цель подсвечивается outline
5. Препятствия:
   - "ящики/реквизит" (легкие)
   - "тяжёлые блоки/горы" (неподвижные, более тёмные/массивные)
   - "зонтики/ширмы" (как деревья, перекрывают обзор и прозрачнеют)
6. Замедляющие зоны - песок/опилки/сыпучка на сцене
...
2. Прицел/направление абилки:
   - мышь: направление на курсор
   - геймпад: направление на правый стик
3. Esc - меню паузы
   - продолжить
...
4. Активная абилка:
   - изначально нет
   - после подбора появляется и работает всегда (push/pull)
...
2. Урон игроку наносится зонами (взрывом) и/или хитбоксами, но не физическим контактом физтел
3. Игрок сталкивается с препятствиями уровня (ящики, тяжёлые блоки, стойки зонтиков)

# 4. Противник (EnemyRoamer)
## 4.1 Визуал
- Клоун-петарда / заводная игрушка
- Визуальные состояния:
  1. бег (chase)
  2. зарядка (arming) - фитиль/мигание + предупреждающий звук
  3. взрыв (explode) - вспышка, кольцо, дым
...
2. Поведение атаки (как “крипер”):
   - при достижении игрока (в радиусе trigger):
     a) замирает  
     b) ждёт `N6 ~ 1 сек` (зарядка)  
     c) наносит урон `N7 ~ 5` по площади (влияет только на игрока по урону), площадь урона равна площади триггера
     d) накладывает замедление всем попавшим в радиус (игрок + противники)  
     e) уничтожается  
   - если игрок отходит через `N6/2` после начала детонации:
     - отменяет взрыв
     - возвращается к погоне
3. Опыт:
   - при смерти от атаки игрока выдаёт опыт `N11 ~ 50`

## 4.4 Замедление от взрыва (общее правило)
1. Взрыв накладывает slow `N8 ~ 15%`, может стакаться
2. Кап: итоговая скорость сущности не может упасть ниже 50% от базовой  
   - (то есть суммарное замедление не более 50%: песок + взрывы + прочее)

## 4.5 Уровни противника (Advanced, не обязательно в MVP версии)
...
2. Уровень даёт 1 из эффектов (по таблице в `leveling_lib/enemy_roamer`):
   ...
   b) уменьшение времени ожидания перед взрывом (`N6`)  
...
### 5.2.1 "Ящики" (легкие препятствия)
...
- Влияние:
  - не сдвигаются, не притягиваются, не отталкиваются
  - блокируют луч абилки
...
## 5.4 Пикап абилки
- Визуал: катушка/крючок, “управление нитями”
- Направление обжектива (маркер): рисуется как иконка рук и платформы с нитями для марианеток, отображается в краю экрана в направлении к пикапу
- Спавн:
...
- Поведение:
  - неподвижен
  - получает урон от базовой атаки игрока
  - может двигаться abилкой push/pull (для мини-пазлов), но не проходит сквозь тяжёлые блоки/границы
- После уничтожения:
...
# 6. Абилка (AbilityString)
## 6.1 Визуал и позиционирование
1. После подбора “катушка” летает вокруг игрока на фиксированном расстоянии
2. Катушка смещается в сторону:
   - курсора мыши
   - или направления правого стика
3. От катушки идёт “нить” (line) до первой физической коллизии

## 6.2 Подсветка цели (outline)
- Нить останавливается на первом объекте с физической коллизией
- Если объект - валидная цель (враг, якорь, легкий ящик):
  - подсвечивается outline
  - показывает игроку, что с этим объектом можно взаимодействовать

## 6.3 Две активные способности
1. ЛКМ - Pull (притягивание к игроку)
2. ПКМ - Push (отталкивание от игрока)

## 6.4 Взаимодействие с объектами и коллизии во время перемещения
1. Для врагов / якорей / легких ящиков:
   - при push/pull объект “скользит” по направлению импульса
   - объект может проходить сквозь лёгкие коллизии (например, других врагов)
   - объект НЕ проходит сквозь:
     a) тяжёлые блоки/горы  
     b) край уровня  
2. При контакте с тяжёлым блоком/краем:
   - импульс прекращается (объект останавливается)
3. Абилка НЕ может двигать тяжёлые блоки/горы

## 6.5 Ограничения абилки (MVP уточнение)
- В MVP важно не сломать навигацию:
  1. если легкие ящики сдвигаются, нужно учитывать что navmesh не перестраивается автоматически
  2. допускается MVP-решение (если не получится обновлять Navmesh): ящики статичны, абилка работает только по врагам/якорям
  3. опция Advanced: динамические препятствия через obstacle avoidance / rebake по событию
...
4. Находит абилку:
   - она помогает контролить толпу и решать мини-пазлы
...
1. “Густые кулисы” (аналог густого леса)
   - много зонтиков/ширм, игрок плохо видит бой, но может разводить врагов и делать bait взрывов
...
3. UI врага:
   ...
   - индикатор зарядки перед взрывом (мигание, кольцо радиуса, звук)
4. UI обжективов:
